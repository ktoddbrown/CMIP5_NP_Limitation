...
for(limitStr in c('N', 'P', 'NP')){
    ...
    for(addSoilN in c(TRUE, FALSE)){
        ...
        if(limitStr %in% 'P'){
            saveFilename <- sprintf('data/%scalcNPPLimit.RData', limitStr)
        }else{
            saveFilename <- sprintf('data/%s%scalcNPPLimit.RData', limitStr, c('_withCsn', '_noCsn')[c(addSoilN, !addSoilN)])
        }
        ...
        ##Only load the biome constraints if we haven't already done so
        if(!('biomeMask' %in% ls())){
        ...
        if(loadPrevious & file.exists(saveFilename)){
            ...
            ##Otherwise intialize the variables we will save
